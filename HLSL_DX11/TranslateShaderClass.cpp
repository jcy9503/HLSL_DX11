...
{
    constexpr WCHAR vsFile[] = L"../HLSL_DX11/TranslateShader/Translate.vs";
    constexpr WCHAR psFile[] = L"../HLSL_DX11/TranslateShader/Translate.ps";
...
bool TranslateShaderClass::Render(ID3D11DeviceContext* deviceContext, const int indexCount,
                                const XMMATRIX worldMatrix, const XMMATRIX viewMatrix, const XMMATRIX projectionMatrix,
                                ID3D11ShaderResourceView** texture, float translation) const
{
    if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture[0], translation))
    {
        return false;
    }
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, const HWND hwnd, const WCHAR* vsFilename,
                                          const WCHAR* psFilename)
{
    ID3D10Blob* errorMessage = nullptr;

    ID3D10Blob* vertexShaderBuffer = nullptr;
    HRESULT result = D3DCompileFromFile(vsFilename, nullptr, nullptr, "TranslateVertexShader",
    ...
    result = D3DCompileFromFile(psFilename, nullptr, nullptr, "TranslatePixelShader",
    ...
    result = device->CreateBuffer(&translateBufferDesc, nullptr, &m_translateBuffer);
    if(FAILED(result)) return false;
...
void TranslateShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, const HWND hwnd,
                                             ...
                                             ID3D11ShaderResourceView* texture, float translation) const
{
    ...
    const auto dataPtr2 = static_cast<TranslateBufferType*>(mappedResource.pData);
...
void TranslateShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, const int indexCount) const
{
